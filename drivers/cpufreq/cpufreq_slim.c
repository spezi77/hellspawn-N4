...
struct timer_list slim_freq_mode_timer;
...
{
	if (timeout > 0)
		mod_timer(&slim_freq_mode_timer, jiffies + msecs_to_jiffies(timeout));
	tbl_select[0] = 2;
	tbl_select[1] = 3;
	tbl_select[2] = 4;
	tbl_select[3] = 4;
...
static void dbs_init_freq_map_table(struct cpufreq_policy *policy)
{
	...
	init_timer(&slim_freq_mode_timer);
	slim_freq_mode_timer.function = switch_mode_timer;
	slim_freq_mode_timer.data = 0;
...
static void dbs_deinit_freq_map_table(void)
{
	...
	del_timer(&slim_freq_mode_timer);
}
